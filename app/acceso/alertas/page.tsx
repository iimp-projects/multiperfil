...
export default function AlertasAdminPage() {
  ...
  const [selectedAlert, setSelectedAlert] = useState<PortalAlertItem | null>(
    null,
  );
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [alertToDelete, setAlertToDelete] = useState<PortalAlertItem | null>(
    null,
  );
  ...
  const fetchAlerts = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/portal/alerts?event=${selectedEvent}`,
      );
  ...
  const fetchGroups = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/portal/groups?event=${selectedEvent}`,
      );
      const data = await res.json();
      if (data.success) {
        setGroups(data.data);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        targetGroup:
          formData.targetMode === "group" ? formData.targetGroup : null,
        recipients:
          formData.targetMode === "specific"
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Gestiona los avisos en tiempo real para los usuarios de{" "}
            {selectedEvent}.
          </p>
        </div>
        <button
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-slate-800">
              {alerts.reduce(
                (acc, curr) => acc + (curr.readBy?.length || 0),
                0,
              )}
            </p>
          </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
          <h2 className="font-bold text-slate-700 flex items-center gap-2">
            ...
            <span className="px-2 py-0.5 bg-slate-200 text-slate-600 rounded-full text-[10px] font-bold uppercase">
              {selectedEvent}
            </span>
          </h2>
        ...
        <div className="divide-y divide-slate-50">
          {isLoading ? (
            ...
            <div className="py-20 flex flex-col items-center justify-center text-slate-400 opacity-60">
              ...
              <p className="text-sm">
                Empieza a comunicarte con tus usuarios creando una nueva alerta.
              </p>
            </div>
                ...
                <div className="flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-slate-500 text-sm line-clamp-1 mb-2">
                      {alert.description || "Sin descripción corta."}
                    </p>
                      ...
                      <span className="flex items-center gap-1">
                        ...
                        {alert.targetGroup
                          ...
                          : alert.recipients?.length > 0
                            ? `${alert.recipients.length} Usuarios`
                            : "Global (Todos)"}
                      </span>
                      <span className="flex items-center gap-1">
            ...
            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
              ...
              >
                  ...
                  <div className="space-y-1">
                    ...
                    >
                      ...
                      <option value="accordion">
                        Acordeón (Contenido oculto)
                      </option>
                      <option value="image_only">
                        Banner (Imagen Clickable)
                      </option>
                    </select>
                  </div>
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button
                        type="button"
                        onClick={() =>
                          setFormData({ ...formData, targetMode: "all" })
                        }
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          setFormData({ ...formData, targetMode: "group" })
                        }
                        className={`flex-1 min-w-[80px] py-3 rounded-xl text-xs font-bold transition-all border ${
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (selectedUsers.length === 0)
                            toast.info(
                              "Ve al módulo de Usuarios para seleccionar destinatarios específicos.",
                            );
                        }}
                        className={`flex-1 min-w-[80px] py-3 rounded-xl text-xs font-bold transition-all border ${
                          formData.targetMode === "specific"
                ...
                <div className="mt-2 p-3 bg-blue-50/50 rounded-xl border border-blue-100/50">
                  ...
                  <ul className="text-sm text-blue-500 space-y-1 list-none p-0 m-0">
                    <li>
                      • <b>Estándar</b>: Solo texto corto en el centro de
                      noticias.
                    </li>
                    <li>
                      • <b>Expandible</b>: Permite ver detalles adicionales al
                      hacer clic.
                    </li>
                    <li>
                      • <b>Modal</b>: Abre una ventana emergente con el
                      contenido.
                    </li>
                    <li>
                      • <b>Acordeón</b>: Contenido colapsable (ideal para FAQs).
                    </li>
                    <li>
                      • <b>Banner</b>: Imagen que ocupa el ancho y redirige al
                      hacer clic.
                    </li>
                  </ul>
                </div>
                ...
                formData.type === "modal" ||
                formData.type === "accordion" ? (
                  <div className="space-y-2 animate-in fade-in slide-in-from-top-2 duration-300">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                      Contenido Extendido (Detalles)
                    </label>
                    <TiptapEditor
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <button
                      type="button"
                      onClick={() =>
                        setFormData({ ...formData, target: "_self" })
                      }
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        setFormData({ ...formData, target: "_blank" })
                      }
                      className={`flex-1 py-3 rounded-xl text-xs font-bold transition-all border ${
      ...
      {selectedAlert && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col animate-in zoom-in-95 duration-300 overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-6 custom-scrollbar">
              <div className="flex gap-4 items-start">
                <div
                  className={`w-12 h-12 rounded-xl shrink-0 flex items-center justify-center shadow-sm ${
                    selectedAlert.variant === "success"
                      ? "bg-emerald-50 text-emerald-500"
                      : selectedAlert.variant === "warning"
                        ? "bg-amber-50 text-amber-500"
                        : selectedAlert.variant === "error"
                          ? "bg-red-50 text-red-500"
                          : "bg-blue-50 text-blue-500"
                  }`}
                >
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="text-sm text-slate-600 mt-1">
                    {selectedAlert.description || "Sin descripción."}
                  </p>
                </div>
                ...
                {selectedAlert.longDescription && (
                  <div>
                    ...
                    <div
                      className="mt-1 p-4 bg-slate-50 rounded-2xl border border-slate-100 text-sm text-slate-600 leading-relaxed prose prose-slate max-w-none portal-message-content"
                      dangerouslySetInnerHTML={{
                        __html: selectedAlert.longDescription,
                      }}
                    />
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-100">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-bold text-slate-700 mt-1">
                      {selectedAlert.targetGroup
                        ? "Grupo Seleccionado"
                        : selectedAlert.recipients?.length > 0
                          ? `${selectedAlert.recipients.length} Usuarios`
                          : "Global (Todos)"}
                    </p>
                  </div>
                </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-black text-blue-800">
                        {selectedAlert.readBy?.length || 0} usuarios
                      </p>
                    </div>
                  </div>
                  <button
          ...
          <div className="flex flex-col items-center text-center space-y-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-slate-500 font-medium leading-relaxed">
                ...
                {alertToDelete?.title}&quot; desaparecerá de todos los buzones
                de los usuarios.
              </p>
            </div>
