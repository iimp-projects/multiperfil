...
export default function StreamingPage() {
  ...
  useEffect(() => {
    const fetchStreaming = async () => {
      ...
      try {
        const res = await fetch(`/api/portal/streaming?event=${eventCode}&userKey=${user.siecode}`);
      ...
      <div className="space-y-8 animate-in fade-in duration-700">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 gap-8">
            {streams.map((stream) => (
              <div key={stream.id} className="bg-white rounded-[2.5rem] overflow-hidden shadow-xl border border-slate-100 flex flex-col xl:flex-row">
                {/* Video Player Area */}
                <div className="flex-1 aspect-video bg-black relative">
                  {stream.vimeoId ? (
                    <iframe
                      src={`https://player.vimeo.com/video/${stream.vimeoId}?h=0&badge=0&autopause=0&player_id=0&app_id=58479`}
                      frameBorder="0"
                ...
                <div className="xl:w-96 p-8 xl:p-10 flex flex-col justify-between bg-slate-50/50">
                  <div>
                    <div className="flex items-center gap-2 mb-4">
                      <span className="px-3 py-1 bg-red-500 text-white text-[10px] font-black rounded-full uppercase tracking-widest animate-pulse">
                        EN VIVO
                      </span>
